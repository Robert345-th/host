...
async function ensureShopFollowsTable() {
  if (!tableReady) {
    tableReady = pool
      ...
      `)
      .then(() =>
        pool.query('CREATE INDEX IF NOT EXISTS shop_follows_shop_id_idx ON shop_follows (shop_id)')
      )
      .catch((err) => {
        tableReady = null;
        throw err;
...
async function getFollowCounts(userId) {
  ...
  const result = await pool.query(
    `SELECT
       (SELECT COUNT(*)::int FROM shop_follows WHERE shop_id = $1) AS followers,
       (SELECT COUNT(*)::int FROM shop_follows WHERE follower_id = $1) AS following`,
    [userId]
  );
  return {
...
function displayName(row) {
  return row.business_name || row.name || 'Vendor';
}

async function notifyShopFollowers(shopId, service) {
  try {
    await ensureShopFollowsTable();
    const shopResult = await pool.query(
      `SELECT COALESCE(NULLIF(business_name, ''), name) AS display_name
       FROM users WHERE id = $1`,
      [shopId]
    );
    const shopName = shopResult.rows[0]?.display_name || 'A shop';
    const followers = await pool.query(
      'SELECT follower_id FROM shop_follows WHERE shop_id = $1 LIMIT 200',
      [shopId]
    );
    const title = String(service.title || 'a new service').slice(0, 80);
      ...
      `SELECT
         ...
         (
           ...
           WHERE s.vendor_id = u.id
             AND s.status = 'active'
             AND s.date_posted > NOW() - INTERVAL '7 days'
         ) AS new_services
       FROM shop_follows f
...
router.get('/counts/:userId', async (req, res) => {
  ...
  try {
    res.json(await getFollowCounts(userId));
  } catch (err) {
    console.error(err);
...
router.get('/status/:shopId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT EXISTS(
         SELECT 1 FROM shop_follows WHERE follower_id = $1 AND shop_id = $2
       ) AS following`,
      [req.userId, shopId]
    );
...
router.post('/:shopId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO shop_follows (follower_id, shop_id)
       VALUES ($1, $2)
...
router.delete('/:shopId', requireAuth, async (req, res) => {
  ...
  try {
    await ensureShopFollowsTable();
    await pool.query(
      'DELETE FROM shop_follows WHERE follower_id = $1 AND shop_id = $2',
      [req.userId, shopId]
    );
    const counts = await getFollowCounts(shopId);
...
module.exports = { router, notifyShopFollowers, ensureShopFollowsTable, getFollowCounts, displayName };
